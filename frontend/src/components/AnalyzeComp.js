...
import {API_GET_ANALYSIS_FILTERED_FIELD} from "../constants";
import axios from "axios";
import {saveAs} from 'file-saver'

const fields = [
    {value: 'author', label: 'Автор'},
    {value: 'museum_name', label: 'Музей'},
    {value: 'start_year', label: 'Год начала'},
    {value: 'end_year', label: 'Год завершения'},
            ...
            <div className='analyzeContainer'>
                <div className='analyzeControls'>
                    ...
                    <Grid container spacing={1} padding={1} alignItems={'center'} className="analyzeGrid">
                        ...
                        <Grid item xs={12}>
                            <TextField size='small' value={filters.author} InputProps={{readOnly: true}}
                                       fullWidth={true}
                                       label="Author" variant="outlined"/>
                        </Grid>
                        <Grid item xs={12}>
                            <TextField size='small'
                                       value={filters.museum_name.localeCompare("Не выбрано") === 0 ? "" : filters.museum_name}
                        ...
                        <Grid item xs={12}>
                            <TextField size='small'
                                       value={filters.genre.localeCompare("Не выбрано") === 0 ? "" : filters.genre}
                                       InputProps={{readOnly: true}} fullWidth={true}
                                       label="Genre" variant="outlined"/>
                        </Grid>
                        <Grid item xs={12}>
                            <TextField size='small'
                                       value={filters.material.localeCompare("Не выбрано") === 0 ? "" : filters.material}
                                       InputProps={{readOnly: true}}
                                       fullWidth={true} label="Materials" variant="outlined"/>
                        </Grid>
                        <Grid item xs={12}>
                            <TextField size='small'
                                       value={filters.type.localeCompare("Не выбрано") === 0 ? "" : filters.type}
                                       InputProps={{readOnly: true}}
                                       fullWidth={true} label="Type" variant="outlined"/>
                        </Grid>
                    </Grid>
                    <Button variant='contained' color='primary' style={{marginTop: 10, marginLeft: 5}}
                            onClick={downloadImage}>Save as .png</Button>
